...
function progress(){const n=Object.values(state.done).filter(Boolean).length;$$('[data-progress]').forEach(el=>el.value=n);$$('[data-progress-label]').forEach(el=>el.textContent=`${n} из 12 уроков отмечено`);const next=Array.from({length:12},(_,i)=>i+1).find(i=>!state.done[i])||12;$$('.resume-link').forEach(a=>a.href=`/lessons/${String(next).padStart(2,'0')}.html`)}
progress();
...
const book=$('#book-frame');if(book){const requested=Number(new URLSearchParams(location.search).get('part')||1),part=[1,2,3,4].includes(requested)?requested:1;const roman=['I','II','III','IV'][part-1],src=`/materials/Barats_1893_part_${roman}.pdf`;book.src=src;book.title=`Курс вексельного права — часть ${roman}`;$('#book-download').href=src;$('#book-open').href=src;$$('[data-book-part]').forEach(a=>{if(Number(a.dataset.bookPart)===part)a.setAttribute('aria-current','page')})}
if(location.pathname==='/'||location.pathname==='/index.html'){const old=location.hash.match(/^#lesson-(\d+)$/);if(old&&+old[1]>=1&&+old[1]<=12)location.replace(`/lessons/${old[1].padStart(2,'0')}.html`)}
